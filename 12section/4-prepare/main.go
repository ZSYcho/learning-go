...
func main() {
	...
	ctx := context.Background() // maybe the context is from the request
	_, err = createUserWithCtx(ctx, db, "Aku Kopd", "akukopd@test.com", "test09120")
	if err != nil {
		log.Fatal(err)
	}
...
func createUserWithCtx(ctx context.Context, db *sql.DB, name, email, hashedPassword string) (int64, error) {
	stmt, err := db.Prepare(`INSERT INTO users (name, email, hashed_password) VALUES ($1, $2, $3)`)
	if err != nil {
		return 0, err
	}
	defer stmt.Close()
...
func createUserWithPrepared(db *sql.DB, name, email, hashedPassword string) (int64, error) {
	...
	// multi execute the statement with different parameters
...
func GetUserByEmail(db *sql.DB, email string) (*User, error) {
	stmt := `SELECT id, name, email, hashed_password, created_at FROM users WHERE email = $1`
...
func GetUsers(db *sql.DB) ([]User, error) {
	stmt := `SELECT id, name, email, ha shed_password, created_at FROM users`
