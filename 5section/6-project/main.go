...
func (he HourlyEmployee) String() string {
	return fmt.Sprintf("Hourly: %s (Rate: %.2f/hr, Hours: %.1f)",
		he.Name, he.HourlyRate, he.HoursWorked)
}
...
func ProcessPayroll(employees []Payable) {
	...
	for _, employee := range employees {
		PrintEmployeeSummary(employee) // Generic print function call
...
func main() {
	...
	hrEmp := HourlyEmployee{Name: "Bob The Builder", HourlyRate: 25.00, HoursWorked: 160}
	comEmp := CommissionEmployee{Name: "Charlie Chaplin", BaseSalary: 2000.00, CommissionRate: 0.10, SalesAmount: 15000}
