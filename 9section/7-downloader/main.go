...
func ConcurrentDownloader(urls []string, destDir string, maxConcurrent int) error {
	...
	for _, url := range urls {
		...
		go func(url string) {
			...
			limiter <- struct{}{}        // this is going to block
			defer func() { <-limiter }() // sort of control of the concurrency, releasing the block
			...
			resp, err := http.Get(url)
			if err != nil {
				results <- Result{URL: url, Error: err} // putting the message in the channel instead returning
				return
			}
			defer resp.Body.Close()
	...
	// start a goroutine to close the results channel when all downloads are done
	go func() {
		wg.Wait()
		close(results) // also terminate the `results` channel block
	}()

	// printing out from results
	var totalSize int64
	var errors []error
	start := time.Now()
...
func main() {
	...
	urls := []string{
		"https://static.zerochan.net/Cyno.full.3766750.png",
		"https://i0.hdslb.com/bfs/new_dyn/6b3737a845ec2c55f4d04dc8b7ba6409842469.jpg",
	}
