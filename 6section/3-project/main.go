...
func (acc *Account) Deposit(amount float64) error { // using the pointer receiver to change the value inside
	if amount <= 0 {
...
func (acc *Account) GetBalance() float64 { // be consistent even though we don't change value inside
	return acc.Balance
}
...
func (sa *SavingsAccount) AddInterest() {
	interest := sa.Balance * sa.InterestRate // Access promoted Balance field
